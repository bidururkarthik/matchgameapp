...
import Tab from './Component/Tab/tabItem'
import ImageList from './Component/ImageList/image'
import './App.css'
...
class App extends Component {
  ...
  getfilterdata = () => {
    ...
    const filterupdate = imagesList.filter(
  ...
  onchangeimage = id => {
    ...
    const getimage = imagesList.filter(each => each.id === id)

    if (getimage[0].imageUrl === randomimage) {
      const num = Math.floor(Math.random() * 31)
      const thubnailimage = imagesList[num].imageUrl
      this.setState({randomimage: thubnailimage})
  ...
  render() {
    const {randomimage, timer, score, result} = this.state
    const filterupdate = this.getfilterdata()
          ...
          {result ? (
            <div className="result_container">
              ...
              <button
                ...
                onClick={this.onchangeplayagain}
              >
                <img
            ...
            <div className="element_container">
              <img src={randomimage} alt="" className="picture" />
              <ul className="tab-list">
                {tabsList.map(eachlist => (
                  <Tab
                    tabdetail={eachlist}
                    key={eachlist.tabId}
                    tabchange={this.onchangetab}
                  />
                ))}
              </ul>

              <ul className="image-list">
                {filterupdate.map(eachimage => (
                  <ImageList
                    eachimage={eachimage}
                    key={eachimage.id}
                    onimage={this.onchangeimage}
                  />
                ))}
              </ul>
            </div>
